...
import { actualizarAutoDeleteVencidos } from '@/lib/actions/user.actions'
import { OPCIONES_AUTO_DELETE_VENCIDOS } from '@/lib/utils/constants'

interface Props {
  valorActual: number | null
}

export function FormularioAutoDeleteVencidos({ valorActual }: Props) {
  ...
  const manejarCambio = (nuevoValor: number | null) => {
    ...
    startTransition(async () => {
      const resultado = await actualizarAutoDeleteVencidos(nuevoValor)
      if (resultado.ok) {
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      >
        {OPCIONES_AUTO_DELETE_VENCIDOS.map((op) => (
