...
**Estado:** Fases 0-34 completadas y en produccion. Fase 34 (jun 10): hero a dos columnas (titulo + demo), features 2+4, categorias con focus fijable por click, y rendimiento — landing estatica/ISR (next-intl movido al layout del dashboard; TTFB de ~1s a CDN), code-splitting bajo el fold, React.cache en getPerfilDelUsuarioActual, favicon-32 e icon.svg declarados. Fase 32 (jun 10): seccion de descargas de las apps nativas en landing y settings (servicio github-releases con cache de 1 h) + ajustes SEO del reporte OG (titulo nuevo, og:url, favicon.ico, descriptions ≤160). Fase 33 (jun 10): refinado visual de la landing — hero compacto con el chat visible en el fold, titular "Sin ruido. Sin olvidos.", strip de confianza mono, features en bento (2 destacadas con mini-visual), Geist Mono real, hover .card-hover consistente y enlace Descargas en el nav. Revision integral de junio 2026 entregada: SEO+landing, seguridad (headers + Upstash), busqueda IA robusta, PWA robusta (retry push + watchdog cron), upgrades (date-fns 4, Zod 4, Next 16 + eslint 9 flat, Tailwind 4, drizzle-kit 0.31.10 — Drizzle 1.0 sigue en RC), limpieza post-upgrades, testing profundo (Fase 29.6) y apps nativas Tauri v2 (Fases 30/31).
...
## 2026-06-28 — Titulos de pestana del navegador

- Las pestanas del dashboard mostraban la marca duplicada ("Inicio | Noti | Noti"): cada page incluia "| Noti" y el template raiz (`%s | Noti` en `layout.tsx`) lo volvia a anadir. Ahora cada pagina del dashboard usa `title: { absolute: '...' }` y muestra solo el nombre de seccion (Inicio, Lanzamientos, Calendario, Notas, Configuracion, {titulo de la nota}).
- La ruta dinamica de categorias (`[slug]`) no declaraba metadata (su pestana caia al default "Noti"): se le anadio `generateMetadata` que resuelve el nombre via `getCategorias()` (Pendientes, Estudio, Cumpleanos, Eventos).
- Paginas publicas (landing/login/register) sin cambios: conservan la marca "| Noti" por SEO/branding.
- Nota de commits: por trabajo concurrente, el title de `settings/page.tsx` quedo incluido en el commit `b375379` de la sesion paralela; los otros 6 archivos van en commit propio. Lint sin nuevos warnings y typecheck limpio en los archivos tocados.

## 2026-06-28 — Instalacion solo en navegador + updater planificado
...
- Nueva seccion "Aplicacion de escritorio" en /settings con 3 toggles: iniciar con el PC (autoarranque), iniciar minimizado en la bandeja, y minimizar a la bandeja al cerrar (antes el cierre a bandeja estaba forzado). Solo visible dentro de la app de escritorio Tauri (no navegador ni Android): el componente cliente se auto-oculta si no hay `window.__TAURI__` desktop o si el invoke inicial falla (build antiguo de la app).
- Rust: comandos IPC `obtener_config_inicio` / `set_autostart` / `set_iniciar_minimizado` / `set_cerrar_a_bandeja` (`src-tauri/src/lib.rs`, todo `#[cfg(desktop)]`). Estado por-maquina en `config.json` local (`src-tauri/src/config.rs`, serde, sin plugin nuevo); el autoarranque se lee/escribe via `tauri-plugin-autostart` (origen de verdad = el SO). Se elimino el `autolaunch().enable()` incondicional: ahora solo en el primer arranque (marca `inicioConfigurado`), luego se respeta la eleccion del usuario. Sin dependencias nuevas (npm o crates).
- Verificado: lint sin nuevos warnings, build de produccion verde. `cargo check` del lado Rust pendiente de validar en `npm run tauri`/CI (cargo no disponible en el entorno local). Verificacion manual en la app instalada pendiente (toggles + reinicio + cierre).
